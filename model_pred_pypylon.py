...
weights = './runs/train/yolov5l_results/weights/best.pt'
conf_thres = 0.25
iou_thres = 0.45
max_det = 20
device = ''
...
if device.type != 'cpu':
    model(torch.zeros(1, 3, pred_shape[1], pred_shape[0]).to(device).type_as(next(model.parameters())))

vid_writer = cv2.VideoWriter('D:/MoveOn/video/ex2.mp4', cv2.VideoWriter_fourcc(*"mp4v"), 5, vis_shape)
while camera.IsGrabbing():
    ...
    if grabResult.GrabSucceeded():
        ...
        image = cv2.resize(image, (pred_shape[1], pred_shape[0]))
        out = cv2.resize(image.copy(), vis_shape)
        ...
        s = datetime.now()
        pred = model(image, augment=False)[0]
